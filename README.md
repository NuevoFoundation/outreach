...
```powershell
node --test .\site\test\email.test.mjs .\site\test\i18n.test.mjs
```

### Languages

The site ships in English, Español, Français, and Português Brasileiro. The
picker matches the one on
[workshops.nuevofoundation.org](https://workshops.nuevofoundation.org/): a plain
`<select class="language-selector">` whose options are written in each language's
own name. English is the base language; the remaining three were prioritized by
the outreach owner.

- **Site language** (header) translates the whole page, sets `<html lang>`, and
  updates the page title.
- **Email language** (step 2) is separate on purpose: a parent who reads Spanish
  may still need to write an English-reading school. It defaults to the site
  language and can be changed independently.
- The choice travels in the address as `?lang=es`, so a link can be shared in a
  chosen language. Nothing is stored in the browser, which keeps the privacy
  promise above. Without that parameter the browser's preferred language is
  used, and regional variants such as `es-MX`, `fr-CA`, or `pt-PT` map to the
  closest shipped language. Anything unrecognized falls back to English.
- `site/i18n.js` holds every translated string. Each language defines exactly the
  same keys, and an automated check fails if one drifts, so no string can
  silently fall back to English.
- Each language has its own flyer page (`flyer-es.html`, `flyer-fr.html`,
  `flyer-pt-br.html`), generated by `node .\site\tools\build-flyers.mjs` from a
  single template. The generator refuses to write if it can no longer reproduce
  the approved English flyer exactly, so translations cannot alter the approved
  layout. Flyer pages stay free of JavaScript, and the published impact figures
  are never restated. Every flyer carries a link row to the other languages,
  which is hidden when printed.
- The email's flyer link automatically points at the matching translated flyer.
- **The translations have not yet been reviewed by native speakers.** Have
  Nuevo Foundation review them before an official public launch.
...
6. Have native speakers review the Spanish, French, and Portuguese translations
   in `site/i18n.js` and `site/tools/build-flyers.mjs`. They were drafted during
   development and are not yet reviewed.
...
| `site/test/i18n.test.mjs` | Language, translation-parity, and translated-flyer checks |
